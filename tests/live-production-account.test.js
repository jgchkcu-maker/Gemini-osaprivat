...
test("live production MCP returns real Gemini critic text after IDE fingerprint update", async () => {
  const response = await fetch(`${BASE_URL}/api/mcp`, {
    ...
    body: JSON.stringify({
      jsonrpc: "2.0",
      id: "live-smoke-after-ide-fingerprint",
      method: "tools/call",
      params: {
        name: "challenge",
        arguments: {
          ...
          context: "This request only checks that production can reach the locked Gemini critic model after the IDE fingerprint update.",
          focus: "general"
        }
      }
    }),
